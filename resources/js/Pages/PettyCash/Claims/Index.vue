...
const statusMeta = {
    my_in_progress: { key: 'my_in_progress', queue: 'my', label: 'In Progress', hint: 'Your petty cash claims still being processed.' },
    my_rejected: { key: 'my_rejected', queue: 'my', label: 'Returned / Rejected', hint: 'Your petty cash claims returned or rejected.' },
    my_completed: { key: 'my_completed', queue: 'my', label: 'Completed', hint: 'Your petty cash claims that are paid.' },
    submitted: { key: 'submitted', queue: 'review', label: 'Needs Check', hint: 'Check by admin.' },
    checked: { key: 'checked', queue: 'review', label: 'Needs Verify', hint: 'Verify by own department head or project department head.' },
    verified: { key: 'verified', queue: 'review', label: 'Needs CEO / GM Approval', hint: 'Verified petty cash claims waiting CEO / GM approval.' },
}
...
const tabStorageKey = 'petty-cash-claims-active-tab'
...
function queueCount(queueKey) {
    if (queueKey === 'my') {
        return sectionCount('my_in_progress') + sectionCount('my_rejected') + sectionCount('my_completed')
    }
...
const currentClaims = computed(() => {
    return claims.value?.[activeTab.value] ?? { data: [], links: [] }
})
...
const browseStatusMeta = {
    all_non_draft: { key: 'all_non_draft', label: 'All Claims', hint: 'Full listing for all petty cash claims.' },
    submitted: { key: 'submitted', label: 'Needs Check', hint: 'Submitted claims waiting for admin check.' },
    checked: { key: 'checked', label: 'Needs Verify', hint: 'Checked claims waiting for department verification.' },
    verified: { key: 'verified', label: 'Needs CEO / GM Approval', hint: 'Verified claims waiting for CEO / GM approval.' },
    rejected: { key: 'rejected', label: 'Returned / Rejected', hint: 'Claims returned or rejected.' },
}

const browseStatuses = ['all_non_draft', 'submitted', 'checked', 'verified', 'rejected']
const browseTabStorageKey = 'petty-cash-claims-browse-tab'
const browseTab = ref(resolveInitialBrowseTab())
const browseClaims = computed(() => {
    return claims.value?.[browseTab.value] ?? { data: [], links: [] }
})
...
<template>
    <AuthenticatedLayout>
        ...
        <div class="p-6 space-y-6">
            ...
            <div v-if="currentClaims?.links?.length" class="flex gap-1">
                <Link
            ...
            <template v-if="canBrowseAllClaims">
                <div class="relative py-2">
                    ...
                    <div class="absolute inset-x-0 -top-1.5 flex justify-center">
                        <span class="rounded-full border border-slate-300 bg-white px-3 py-0.5 text-[11px] font-semibold uppercase tracking-wide text-slate-600">
                            All Petty Cash Claims Browse Section
                        </span>
                    </div>
                </div>
                ...
                <div class="space-y-4 rounded-xl border-2 border-slate-300 bg-slate-50/60 p-4">
                    <div>
                        <h3 class="text-base font-semibold text-slate-900">All Petty Cash Claims Browse</h3>
                        <p class="text-sm text-slate-600">Filter and browse all petty cash claims.</p>
                    </div>

                    <StandardFilterBar
                        title="Browse Filters"
                        description="Use filters below for full petty cash claims listing."
                        @apply="applyFilters"
                        @reset="resetFilters"
                    >
                    ...
                    <div v-if="browseClaims?.links?.length" class="flex gap-1">
                        <Link
