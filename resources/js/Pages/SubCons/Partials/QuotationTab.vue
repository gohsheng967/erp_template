...
async function uploadQuotations() {
    ...
    try {
        await axios.post(
            route('sub-cons.purchase-quotations.upload', props.subCon.uuid),
            form
        )

        toast?.value?.show(`${uploadItems.value.length} quotation(s) uploaded`, 'success')
        uploadItems.value = []
        router.reload({ preserveScroll: true })
    } catch (err) {
        toast?.value?.show(err?.response?.data?.message || 'Upload failed', 'error')
    } finally {
...
async function confirmDeleteQuotation() {
    ...
    try {
        await axios.delete(
            route('sub-cons.purchase-quotations.destroy', [
                props.subCon.uuid,
        ...
        toast?.value?.show(err?.response?.data?.message || 'Delete failed', 'error')
    } finally {
...
function handleAddSuccess() {
    ...
    selected.value = []
    toast?.value?.show('Quotation(s) added to Purchase Request', 'success')
    router.reload({ preserveScroll: true })
}
</script>
...
<template>
    <div class="space-y-6">
            ...
            >
                <div class="flex justify-between items-center mb-3">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-medium truncate">{{ q.file.name }}</span>
                    </div>
                    <button
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-2">
                    <input v-model="q.amount" type="number" step="0.01" placeholder="Amount *" class="border rounded-md px-2 py-1.5 text-sm" />
                    <input v-model="q.quotation_no" type="text" placeholder="Quotation No *" class="border rounded-md px-2 py-1.5 text-sm" />
                    <input v-model="q.delivery_time" type="number" placeholder="Delivery" class="border rounded-md px-2 py-1.5 text-sm" />
                    <textarea v-model="q.terms" rows="1" placeholder="Terms" class="border rounded-md px-2 py-1.5 text-sm resize-none"></textarea>
                </div>
            </div>
        ...
        <div v-if="selected.length" class="flex justify-end mb-3">
            <button
                ...
                <tbody>
                    <tr v-for="q in (quotations.data ?? [])" :key="q.id" class="border-t">
                        ...
                        <td class="px-4 py-2 text-center">
                            ...
                            <span v-else class="text-gray-300" title="No notes">
                                <i class="mdi mdi-note-off-outline text-lg"></i>
                            </span>
                        </td>
                        <td class="text-center py-2">
                            <span v-if="q.pr_count > 0" class="px-2 py-1 bg-indigo-100 text-indigo-700 rounded-full text-xs">
                                {{ q.pr_count }}
                            </span>
        ...
        >
            <div class="bg-white rounded-lg shadow-xl w-full max-w-lg">
                <div class="flex items-center justify-between px-6 py-4 border-b">
                    <h3 class="text-lg font-semibold text-gray-800">Quotation Notes</h3>
                    <button class="text-gray-400 hover:text-gray-600" @click="showTerms = false">
                        x
                    </button>
                </div>
                <div class="px-6 py-6">
                    <pre class="whitespace-pre-wrap text-sm text-gray-700">{{ activeTerms || '-' }}</pre>
                </div>
                <div class="flex justify-end px-6 py-4 border-t bg-gray-50">
                    <button class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300" @click="showTerms = false">
                        Close
                    </button>
                </div>
            </div>
