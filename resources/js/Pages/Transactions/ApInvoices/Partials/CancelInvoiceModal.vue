...
function submit() {
    ...
    form.post(
        route('ap-invoices.cancel', props.invoice.uuid),
        {
            preserveScroll: true,
            onSuccess: () => {
                toast?.value?.show('Invoice cancelled successfully', 'success')
                emit('success')
                close()
            },
            onError: () => {
                toast?.value?.show('Failed to cancel invoice', 'error')
            },
        }
    )
}
    ...
    >
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md z-10">
            ...
            <div class="p-6 space-y-4">
                    ...
                    <div class="flex items-start gap-2">
                        ...
                        <div>
                            ...
                            <ul class="list-disc ml-5 mt-1 space-y-1">
                                ...
                                <li>Cancel all related payments</li>
                                <li>Set invoice balance to zero</li>
                            </ul>
                        </div>
                    </div>
                </div>
                ...
                <!-- INVOICE INFO -->
                <div class="text-sm space-y-1 bg-gray-50 border rounded-lg p-3">
            ...
            <div class="px-6 py-4 border-t flex justify-end gap-3 bg-gray-50 rounded-b-xl">
                ...
                >
                    {{ form.processing ? 'Cancelling…' : 'Cancel Invoice' }}
                </button>
